...
SEcategory_demo_3 <- function(se_seg_df,e_fit) {
  ...
  e_fit_filter <- e_fit[which(e_fit$se_merge_name %in% se_name),]
  ...
  for (se_index in c(1:length(se_name))) {
    ...
    temp_se_fit <- e_fit_filter[which(e_fit_filter$se_merge_name==se_name[se_index]),]
    #temp_seg_df <- se_seg_filter[which(se_seg_filter$se_merge_name == "chr19_41859354_41885079"),]
    #temp_se_fit <- e_fit_filter[which(e_fit_filter$se_merge_name=="chr19_41859354_41885079"),]

    # if nrow > 6, just consider top 95% points
    if (nrow(temp_se_fit) >= 6) {
      temp_se_fit <- temp_se_fit[which(temp_se_fit$cumsum < 96),]
    } else {
      temp_se_fit <- temp_se_fit
    }
    ...
    if (unique(temp_cat_df$seg_number) == 1) {
      if (temp_cat_df$seg_loc == "mid") {
        # get boundray point and max or min middle point to define the pattern use temp_se_fit
        l_point <- head(temp_se_fit$spline_bs[order(temp_se_fit$width_mid)],n=1)
        r_point <- tail(temp_se_fit$spline_bs[order(temp_se_fit$width_mid)],n=1)
        max_point <- max(temp_se_fit$spline_bs)
        min_point <- min(temp_se_fit$spline_bs)

        # no peak or straight line
        if (max_point %in% c(l_point,r_point) &
            min_point %in% c(l_point,r_point)) {
          if (abs(max_point-min_point) <= 0.5) {
            ...
            # direction
            if (abs(max_point) > abs(min_point) & max_point > 0) {
              temp_cat_df$direction <- "+"
            } else if (abs(max_point) < abs(min_point) & min_point < 0) {
              temp_cat_df$direction <- "-"
            }
          ...
          # have 1 peak
        } else if (max_point %in% c(l_point,r_point) & !min_point %in% c(l_point,r_point) |
            !max_point %in% c(l_point,r_point) & min_point %in% c(l_point,r_point)) {
          # check max or min point
          flag <- which(!c(min_point,max_point)%in%c(l_point,r_point))

          if (abs(max_point-min_point) <= 0.5) {
            temp_cat_df$category <- rep("Similar",nrow(temp_cat_df))
            temp_cat_df$direction <- "none"
          } else if (abs(max_point-min_point) >0.5 &
                     flag == 1 &
                     abs(min_point) <= 0.5 ) {
            temp_cat_df$category <- rep("Shorten",nrow(temp_cat_df))
            temp_cat_df$direction <- "+"
