...
*SE_category, pattern_list, se_fit* are the output regarding to SE internal dynamics.
...
You can also chose the permutation type with *permut_type* parameter. This parameter takes string *normal* or *stringent*. *Stringent* is used for comparing two similar conditions, like before and after drug treatment of a same cell.
...
DASE_out <- DASE(se_in=se_region,e_in=enhancer_region,bl_file = blacklist_region,
                 custom_range = c("chr21:1000-100000","chr21:123456-250000"),
                 enhancer_count_table=enhancer_count)

```

You can find that the number of SEs is less than without blacklist region (<).

## *DASE with no permutation*
Some examples of using permutation options. Here, we focus on using DASE with enhancer count table.
```{r permut_s}
# run DASE with stringent permutation
DASE_out <- DASE(se_in=se_region,e_in=enhancer_region,permut = T,times = 1,
                 permut_type = "stringent",
                 enhancer_count_table=enhancer_count)
DASE_out$density_plot
```

You can see the permuted density plot is just a vertical line with 0. This setting can only be used for the comparison of similar conditions. Because our demo dataset is two different cell line (K562 vs. MCF7), *permut_type='stringent'* will not work.

```{r no_permut}
# run DASE with no permutation and customized threshold
DASE_out <- DASE(se_in=se_region,e_in=enhancer_region,permut = F,
                 cutoff_v = c(-2,2),enhancer_count_table=enhancer_count)
DASE_out$boxplot
```

Because we have a larger threshold, more SEs are identified as *similar* category than before.
